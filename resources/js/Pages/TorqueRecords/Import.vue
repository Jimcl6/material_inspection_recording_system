...
const previewImport = async () => {
    ...
    try {
        const response = await axios.post(route('torque-records.import.preview'), formData, {
            headers: {
...
const executeImport = async () => {
    ...
    try {
        const response = await axios.post(route('torque-records.import.execute'), {
...
<template>
    <Head title="Import Torque Checksheet" />

    <AppLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    Import Torque Checksheet
                </h2>
                <Link 
                    ...
                    <div class="p-6">
                        ...
                        <div v-if="hasDuplicates" class="mb-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            ...
                            <p class="text-sm text-yellow-700 mb-3">
                                {{ previewResults?.duplicate_records?.length }} record(s) already exist in the database with matching date, screw type, process, and line.
                            </p>
                                    ...
                                    <table class="min-w-full text-xs">
                                        <thead class="bg-yellow-100">
                                            <tr>
                                                ...
                                                <th class="px-2 py-1 text-left">Screw Type</th>
                                        ...
                                        <tbody>
                                            <tr v-for="(dup, index) in previewResults?.duplicate_records" :key="index" class="border-b border-yellow-100">
                                                ...
                                                <td class="px-2 py-1">{{ dup.new_data.screw_type }}</td>
                                                <td class="px-2 py-1">{{ dup.new_data.process_assigned }}</td>
                                                <td class="px-2 py-1">{{ dup.new_data.line_assigned }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                    ...
                                    <table class="min-w-full text-xs">
                                        <thead class="bg-gray-100">
                                            <tr>
                                                ...
                                                <th class="px-2 py-1 text-left">Screw Type</th>
                                                <th class="px-2 py-1 text-left">Process</th>
                                                <th class="px-2 py-1 text-left">Torque AM</th>
                                                <th class="px-2 py-1 text-left">Torque PM</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr v-for="(rec, index) in previewResults?.new_records" :key="index" class="border-b border-gray-100">
                                                <td class="px-2 py-1">{{ rec.date }}</td>
                                                <td class="px-2 py-1">{{ rec.screw_type }}</td>
                                                <td class="px-2 py-1">{{ rec.process_assigned }}</td>
                                                <td class="px-2 py-1">{{ rec.torque_am }}</td>
                                                <td class="px-2 py-1">{{ rec.torque_pm }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                            ...
                            <ul class="text-sm text-blue-700 list-disc list-inside space-y-1">
                                <li>Use the HPI-PR02-01 TORQUE CHECKSHEET format</li>
                                <li>Model series and line are read from rows 2-3</li>
                                <li>Dates are detected from row 6 (format: "Date : DD/MM/YYYY")</li>
                                <li>Each screw type block spans 6 rows starting from row 10</li>
                                <li>Master/template sheets will be skipped automatically</li>
                                <li>Duplicates are detected by date + screw type + process + line</li>
                            </ul>
                        </div>
