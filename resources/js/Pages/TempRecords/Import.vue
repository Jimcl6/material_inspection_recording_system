...
interface PreviewRecord {
    ...
    machine_setting_standard: string;
    temp_am: string;
    temp_pm: string;
...
interface PreviewResults {
    ...
    total_parsed: number;
    errors: string[];
    detected_equipment_type: string | null;
}
...
const equipmentType = ref('Soldering Iron');
const lineAssigned = ref('');
const processAssigned = ref('');
const detectedEquipmentType = ref<string | null>(null);

const equipmentTypeOptions = ['Soldering Iron', 'Soldering Pot'];
...
const handleFileChange = (event: Event) => {
    ...
    if (target.files && target.files.length > 0) {
        ...
        detectedEquipmentType.value = null;
    }
};
...
const previewImport = async () => {
    ...
    try {
        const response = await axios.post(route('temp-records.import.preview'), formData, {
            headers: {
        ...
        if (response.data.success) {
            ...
            // Update detected equipment type if available
            if (response.data.preview.detected_equipment_type) {
                detectedEquipmentType.value = response.data.preview.detected_equipment_type;
                // Auto-select detected type if user hasn't changed it
                if (equipmentType.value === 'Soldering Iron' && detectedEquipmentType.value) {
                    equipmentType.value = detectedEquipmentType.value;
                }
            }
        } else {
...
const executeImport = async () => {
    if (!equipmentType.value) {
        errorMessage.value = 'Please select an equipment type.';
        return;
    }
    ...
    try {
        const response = await axios.post(route('temp-records.import.execute'), {
            equipment_type: equipmentType.value,
            line_assigned: lineAssigned.value || null,
            process_assigned: processAssigned.value || null,
...
const resetForm = () => {
    ...
    errorMessage.value = '';
    successMessage.value = '';
    equipmentType.value = 'Soldering Iron';
    lineAssigned.value = '';
    processAssigned.value = '';
    detectedEquipmentType.value = null;
    if (fileInput.value) {
...
<template>
    <Head title="Import Temperature Records" />
                    ...
                    <div class="p-6">
                        ...
                        <div v-if="hasDuplicates" class="mb-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            ...
                            <p class="text-sm text-yellow-700 mb-3">
                                {{ previewResults?.duplicate_records?.length }} record(s) already exist in the database with matching date, equipment type, and control number.
                            </p>
                                    ...
                                    <table class="min-w-full text-xs">
                                        <thead class="bg-yellow-100">
                                            <tr>
                                                ...
                                                <th class="px-2 py-1 text-left">Equipment Type</th>
                                                <th class="px-2 py-1 text-left">Control No</th>
                                                <th class="px-2 py-1 text-left">Temp AM</th>
                                                <th class="px-2 py-1 text-left">Temp PM</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr v-for="(dup, index) in previewResults?.duplicate_records" :key="index" class="border-b border-yellow-100">
                                                <td class="px-2 py-1">{{ dup.new_data.date }}</td>
                                                <td class="px-2 py-1">{{ dup.new_data.equipment_type }}</td>
                                                <td class="px-2 py-1">{{ dup.new_data.solder_model }}</td>
                                                <td class="px-2 py-1">{{ dup.new_data.temp_am }}</td>
                                                <td class="px-2 py-1">{{ dup.new_data.temp_pm }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                    ...
                                    <table class="min-w-full text-xs">
                                        <thead class="bg-gray-100">
                                            <tr>
                                                ...
                                                <th class="px-2 py-1 text-left">Setting Std</th>
                                                <th class="px-2 py-1 text-left">Temp AM</th>
                                                <th class="px-2 py-1 text-left">Temp PM</th>
                                        ...
                                        <tbody>
                                            <tr v-for="(rec, index) in previewResults?.new_records" :key="index" class="border-b border-gray-100">
                                                ...
                                                <td class="px-2 py-1">{{ rec.model_series }}</td>
                                                <td class="px-2 py-1">{{ rec.machine_setting_standard }}</td>
                                                <td class="px-2 py-1">{{ rec.temp_am }}</td>
                                                <td class="px-2 py-1">{{ rec.temp_pm }}</td>
                                                <td class="px-2 py-1">{{ rec.person_in_charge }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <div class="mb-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
                            <h4 class="font-medium text-gray-800 mb-4">Import Settings</h4>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    >
                                        <option v-for="opt in equipmentTypeOptions" :key="opt" :value="opt">
                                            {{ opt }}
                                        </option>
                                    </select>
                                    <p v-if="detectedEquipmentType" class="mt-1 text-xs text-green-600">
                                        Auto-detected: {{ detectedEquipmentType }}
                                    </p>
                                </div>
                        ...
                        <div class="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                            ...
                            <ul class="text-sm text-blue-700 list-disc list-inside space-y-1">
                                <li>Use the HPI-PR05-01 SOLDERING TEMPERATURE CONTROL format</li>
                                <li>Equipment No. and Machine Setting Standard are read from row 4</li>
                                <li>Dates are detected from row 6 (Excel date format)</li>
                                <li>Time Checked from row 8, Actual Temperature from row 9</li>
                                <li>MODEL CODE, P.I.C., and Checked by from rows 80-82</li>
                                <li>Duplicates are detected by date + equipment type + control number</li>
                            </ul>
                        </div>
