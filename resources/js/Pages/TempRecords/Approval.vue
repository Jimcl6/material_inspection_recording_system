...
const submit = (action) => {
    ...
    form.post(route(action === 'approve' ? 'temp-records.bulk-approve' : 'temp-records.bulk-reject'), {
        onSuccess: () => {
            selectedIds.value = [];
            notes.value = '';
            form.reset();
...
<template>
    <Head title="Approve Temperature Records" />

    <AppLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Temperature Record Approval</h2>
                <Link :href="route('temp-records.index')" class="text-gray-600 hover:text-gray-800">
                    &larr; Back to List
                </Link>
            </div>
        </template>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <TabletApprovalReview
                        ...
                        :subtitle-for="(record) => record.control_no"
                        :facts-for="(record) => [
                        ...
                        show-route-name="temp-records.show"
                    ...
                    <div class="mb-4 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                        <div>
                            <h3 class="text-lg font-medium text-gray-900">Pending Temperature Records</h3>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Control No.</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="record in pendingRecords" :key="record.id">
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-500">{{ record.equipment_type || 'N/A' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-500">{{ record.control_no || 'N/A' }}</td>
                                    ...
                                    <td class="px-4 py-3 text-right text-sm">
                                        <Link :href="route('temp-records.show', record.id)" class="text-indigo-600 hover:text-indigo-900">View</Link>
                                    </td>
                                </tr>
                                <tr v-if="!pendingRecords.length">
                                    <td colspan="8" class="px-4 py-6 text-center text-sm text-gray-500">No pending temperature records.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
