...
const previewImport = async () => {
    ...
    try {
        const response = await axios.post(route('diaphragm-welding.import.preview'), formData, {
            headers: {
...
const executeImport = async () => {
    ...
    try {
        const response = await axios.post(route('diaphragm-welding.import.execute'), {
                    ...
                    <div class="p-6">
                        ...
                        <div v-if="hasDuplicates" class="mb-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            ...
                            <p class="text-sm text-yellow-700 mb-3">
                                {{ previewResults?.duplicate_records?.length }} record(s) already exist in the database with matching production date and lasermark lot number.
                            </p>
                                ...
                                <div class="mt-2 max-h-60 overflow-y-auto">
                                    <table class="min-w-full text-xs">
                                        <thead class="bg-yellow-100">
                                            <tr>
                                                <th class="px-2 py-1 text-left">Production Date</th>
                                                <th class="px-2 py-1 text-left">Lasermark Lot</th>
                                                <th class="px-2 py-1 text-left">Machine #</th>
                                                <th class="px-2 py-1 text-left">JO Number</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr v-for="(dup, index) in previewResults?.duplicate_records" :key="index" class="border-b border-yellow-100">
                                                ...
                                                <td class="px-2 py-1">{{ dup.new_data.lasermark_lot_number }}</td>
                                                <td class="px-2 py-1">{{ dup.new_data.machine_no }}</td>
                                                <td class="px-2 py-1">{{ dup.new_data.jo_number }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                    ...
                                    <table class="min-w-full text-xs">
                                        ...
                                        <tbody>
                                            <tr v-for="(rec, index) in previewResults?.new_records" :key="index" class="border-b border-gray-100">
                                                ...
                                                <td class="px-2 py-1">{{ rec.lasermark_lot_number }}</td>
                                                <td class="px-2 py-1">{{ rec.machine_no }}</td>
                                                <td class="px-2 py-1">{{ rec.jo_number }}</td>
                                                <td class="px-2 py-1">{{ rec.prod_qty }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                            ...
                            <ul class="text-sm text-blue-700 list-disc list-inside space-y-1">
                                <li>Use the same Excel format as the reference file</li>
                                ...
                                <li>Duplicates are detected by Production Date + Lasermark Lot Number</li>
                            </ul>
                            <p class="mt-2">
                                <a 
                                    :href="route('diaphragm-welding.export')" 
